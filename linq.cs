   // projection records are needed since i design a wrong database relationship
                //the only way i can do is reformat the object 
                //this make poor performance but saving memory 
                ...
                var CalProfit = (from r in ProjectionRecords //all r are groups by size
                        ...
                        let goatOri = goatCollection.Any()
                            ? r.Where(x => x.records.goatprice > 0).Average(c => (double) c.records.goatprice)
                            : 0
                ...
                var fuckedProducts =
                    CalProfit.Where(c => c.duoPrice == 0 || c.goPrice == 0); //either one no product sell = fucked up
                var statistics = new StatisticsModel
                {
                    ...
                    avgNeg = negQuery.Average(c => c.profit),
                    nPos = posQuery.Count(),
                    nNeg = negQuery.Count()
                };
                //after do projection , calProfit, statistics , we can make them ToList , then do json seriilzer , 
                //keep in mind that we must project the sql to memory objects before directly map to json
